...
- `sql/post_purchase_analysis.sql`
- SQL은 MySQL Workbench에서 실행했다.
- 데이터는 `sql/seed_data.sql`로 생성한 샘플 데이터다.

### 주요 결과

전체 주문 수는 3건이고, 리뷰가 작성된 주문 수는 3건이다.

주문 기준 리뷰 작성률은 100.00%로 확인되었다.

리뷰 작성까지 걸린 시간은 다음과 같다.

| review_id | minutes_to_review |
|---|---:|
| `r001` | 3 |
| `r003` | 3 |
| `r002` | 3228 |

`r001`과 `r003`은 구매와 같은 세션에서 작성된 리뷰다.

`r002`는 구매 이후 별도 세션에서 작성된 리뷰다.

상품별 리뷰 수와 평균 평점은 다음과 같이 확인되었다.
...
리뷰가 없는 상품은 `review_count` 0, `avg_rating` NULL로 조회되었다.

사용자별로는 `u001`, `u002`, `u004`가 각각 주문 1건과 리뷰 1건을 가지고 있다. `u003`과 `u005`는 주문이 없어 `review_rate`가 NULL로 조회되었다.

### 결과 해석

현재 샘플 데이터에서는 모든 주문이 리뷰 작성으로 이어졌다.

구매 이후 행동은 같은 구매 세션에서 즉시 발생한 리뷰와, 별도 세션에서 나중에 발생한 리뷰로 구분할 수 있었다.

특히 `r002`는 구매 이후 약 3228분 뒤 별도 세션에서 작성된 리뷰로, 구매 이후 재방문 행동을 표현하는 사례로 볼 수 있다.

이를 통해 구매 이후 행동 분석에서는 리뷰 작성 여부뿐 아니라 리뷰 작성 시점과 세션 유형을 함께 보는 것이 중요함을 확인했다.

### 한계

- 현재 데이터는 `sql/seed_data.sql` 기반 샘플 데이터이므로 실제 리뷰 작성률이나 평균 평점으로 일반화할 수 없다.
- 본 결과는 구매 이후 행동 분석 쿼리가 정상적으로 작동함을 확인한 결과로 해석한다.

### 다음 분석 방향

- 더 큰 synthetic data를 생성해 리뷰 작성률과 리뷰 작성까지 걸린 시간을 분포로 분석한다.
- 구매 이후 재방문 세션과 같은 세션 리뷰 작성 행동을 구분해 비교한다.
